...
void HWCodecPlayer::Init(JNIEnv *jniEnv, jobject obj  ,jstring  jurl , char *url, int videoRenderType, jobject surface) {

   //const char *videoPath = jniEnv->GetStringUTFChars(url2, 0);
...
//    av_register_all();
//
//    LOGCATE("Retrieve stream information");
//    if (avformat_find_stream_info(m_AVFormatContext, NULL) < 0) {
//        LOGCATE("Cannot find stream information.");
//        return;
//    }
//
//    // Find the first video stream
//    LOGCATE("Find video stream");
...
//        LOGCATE("No video stream found.");
//        return; // no video stream found.
//    }
//
//    // Get a pointer to the codec context for the video stream
//    LOGCATE("Get a pointer to the codec context for the video stream");
//    AVCodecParameters *codecParameters = m_AVFormatContext->streams[video_stream_index]->codecpar;
//
//    // Find the decoder for the video stream
//    LOGCATE("Find the decoder for the video stream");
...
//        LOGCATE("Init CodecContext failed.");
//        return;
//    }
//
...
//    if (packet == NULL) {
//        LOGCATE("Could not allocate av packet.");
//        return;
//    }
//
//    // Allocate video frame
//    LOGCATE("Allocate video frame");
//    AVFrame *frame = av_frame_alloc();
//    // Allocate render frame
//    LOGCATE("Allocate render frame");
...
//    LOGCATE("Determine required buffer size and allocate buffer");
//    int size = av_image_get_buffer_size(dstFormat, codecContext->width,
...
//                                                   NULL);
//    if (swsContext == NULL) {
//        LOGCATE("Init SwsContext failed.");
//        return;
//    }
//
//    // native window
//    LOGCATE("native window");
////            ANativeWindow *nativeWindow = ANativeWindow_fromSurface(environ, surface);
//    ANativeWindow_Buffer windowBuffer;
//
//    // get video width , height
//    LOGCATE("get video width , height");
//    int videoWidth = codecContext->width;
//    int videoHeight = codecContext->height;
//    LOGCATE("VideoSize: [%d,%d]", videoWidth, videoHeight);
//
//    // 设置native window的buffer大小,可自动拉伸
//    LOGCATE("set native window");
//    ANativeWindow_setBuffersGeometry(m_ANativeWindow, videoWidth, videoHeight,
//                                     WINDOW_FORMAT_RGBA_8888);
//
//
//    LOGCATE("read frame");
//    while (av_read_frame(m_AVFormatContext, packet) == 0) {
...
//                    ANativeWindow_lock(m_ANativeWindow, &windowBuffer, NULL);
//
//                    // 格式转换
...
//                    LOGCATE("从解码器-接收-数据失败：AVERROR(EAGAIN)");
//                } else if (receiveFrameState == AVERROR_EOF) {
//                    LOGCATE("从解码器-接收-数据失败：AVERROR_EOF");
//                } else if (receiveFrameState == AVERROR(EINVAL)) {
//                    LOGCATE("从解码器-接收-数据失败：AVERROR(EINVAL)");
//                } else {
//                    LOGCATE("从解码器-接收-数据失败：未知");
//                }
//            } else if (sendPacketState == AVERROR(EAGAIN)) {//发送数据被拒绝，必须尝试先读取数据
//                LOGCATE("向解码器-发送-数据包失败：AVERROR(EAGAIN)");//解码器已经刷新数据但是没有新的数据包能发送给解码器
//            } else if (sendPacketState == AVERROR_EOF) {
//                LOGCATE("向解码器-发送-数据失败：AVERROR_EOF");
//            } else if (sendPacketState == AVERROR(EINVAL)) {//遍解码器没有打开，或者当前是编码器，也或者需要刷新数据
//                LOGCATE("向解码器-发送-数据失败：AVERROR(EINVAL)");
//            } else if (sendPacketState == AVERROR(ENOMEM)) {//数据包无法压如解码器队列，也可能是解码器解码错误
//                LOGCATE("向解码器-发送-数据失败：AVERROR(ENOMEM)");
//            } else {
//                LOGCATE("向解码器-发送-数据失败：未知");
//            }
//
//        }
...
//    avformat_free_context(m_AVFormatContext);
 //    jniEnv->ReleaseStringUTFChars(url2, videoPath);
}
...
void HWCodecPlayer::Play() {
    ...
    if(m_DeMuxThread == nullptr) {
        LOGCATE("HWCodecPlayer::Play1");
        m_DeMuxThread = new thread(DeMuxThreadProc, this);
    } else {
        LOGCATE("HWCodecPlayer::Play2");
...
int HWCodecPlayer::InitDecoder() {
    ...
    int result = -1, StreamIndex = 0;
    int result2 = -1;
    AVDictionary* dict = nullptr;
    AVCodecContext *codecContext = nullptr;


    av_dict_set(&dict,"timeout","10000000",0);
    av_dict_set(&dict,"-max_Delay","0",0);
    av_dict_set(&dict,"pkt_size","4119552",0);
    av_dict_set(&dict,"buffer_size","4119552",0);
    av_dict_set(&dict, "protocol_whitelist", "tcp", 0);
    ...
    try {
        while (true) {
            result2 = avformat_open_input(&m_AVFormatContext, "tcp://103.182.65.183:9001",
                                          nullptr, &dict);
            if (result2 < 0) {
...
//
            ...
            avformat_close_input(&m_AVFormatContext);
            avformat_free_context(m_AVFormatContext);
            jniEnv_->ReleaseStringUTFChars(jUrl_, url_);
        }
    }
    catch (exception e){
    }
...
//
//            while (true) {
//                result = av_read_frame(m_AVFormatContext, packet);
//                if (result != 0) {
//                    break;
//                }
...
//                        int frameCount = 0;
//                        // show frame to use
//                        LOGCATE("HWCodecPlayer::InitDecoder avformat_open_input frameeee", frame);
//
//
//                    }
//                    //判断一个 packet 是否解码完成
...
void HWCodecPlayer::UpdateTimeStamp() {
