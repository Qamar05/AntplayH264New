...
void PlayerWrapper::Init(JNIEnv *jniEnv, jobject obj  , jstring jurl , char *url, int playerType, int renderType,
                         jobject surface) {
    switch (playerType) {
        case FFMEDIA_PLAYER:
    ...
    if(m_MediaPlayer)
        m_MediaPlayer->Init(jniEnv, obj  ,jurl ,  url, renderType, surface);
}
